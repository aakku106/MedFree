...
try {
  ...
  if (fs.existsSync(targetEnvFile)) {
    ...
    if (envContent.includes('NEXT_PUBLIC_VAPID_PUBLIC_KEY')) {
      ...
      console.log('   If you want to replace them, manually update the values above.\n');
    } else {
      // Prompt to add keys
      console.log('💡 Would you like to automatically add these keys to your .env file?');
      console.log('   Press Ctrl+C to cancel, or run this command:\n');
      console.log(`   node -e "const fs = require('fs'); const content = fs.readFileSync('${targetEnvFile}', 'utf8'); fs.writeFileSync('${targetEnvFile}', content + '\\n# Push Notifications (VAPID)\\nNEXT_PUBLIC_VAPID_PUBLIC_KEY=${vapidKeys.publicKey}\\nVAPID_PRIVATE_KEY=${vapidKeys.privateKey}\\nVAPID_EMAIL=admin@medfree.com\\n');"`);
      console.log('');
    }
  } else {
    console.log('⚠️  No .env file found. Please create one and add the keys above.\n');
  }
