...
export async function subscribeToPush(registration, vapidPublicKey) {
  try {
    ...
    if (!vapidPublicKey || vapidPublicKey === "your_public_key") {
      throw new Error(
        "Invalid VAPID public key. Please run: npx web-push generate-vapid-keys and update your .env file"
      );
    }
    ...
    if (vapidPublicKey.length < 80) {
      throw new Error(
        "VAPID public key appears to be invalid (too short). It should be an 88-character base64 string."
      );
    }
    ...
    if (error.name === "InvalidAccessError") {
      throw new Error(
        "Invalid VAPID key format. Please verify your NEXT_PUBLIC_VAPID_PUBLIC_KEY in .env is a valid P-256 public key from: npx web-push generate-vapid-keys"
      );
    }
